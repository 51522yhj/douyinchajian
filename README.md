...
- 应用首页会显示最近一次事件包名、最近一次抖音包名、扫描结果、动作和错误信息，用来判断服务是否真的读到了抖音里的章节标记。
- 开启无障碍快捷方式后，点击右侧无障碍快捷按钮会在当前画面底部显示最近一次诊断浮层，方便在抖音播放页直接查看完整状态，不再用系统 Toast 截断显示。
- 如果只读到“章节要点/分段”但没有读到“推广/广告”标签，当前版本会先尝试跳到下一分段，用来验证安卓端的分段跳转手势是否有效。
- 服务只会在当前活动窗口本身也是抖音包名时扫描和拖动，避免误扫插件自己的说明页面或系统设置页。
- 诊断浮层内有“测试跳段”按钮，会直接在当前抖音视频页执行多策略进度条拖动，用于验证手势是否能控制抖音进度条。
- 为避免 MIUI 系统无障碍快捷按钮不触发回调，服务会显示一个右侧“诊”按钮；点击它可打开诊断浮层。
